...
saveTransactionInfo:{[Block]
  ...
  insert[`txidLookup;select txid,height,partition:heightToPartition[index;lookupChunkSize],parted:`$-2#'txid from tx];
  insert[`txInfo;tx];
 }
...
saveTransactionOutputs:{[Block]
  ...
  insert[`addressLookup;select address,height,partition:heightToPartition[index;lookupChunkSize],parted:`$-2#'address from outputs where not address like ""];
